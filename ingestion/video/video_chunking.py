...
def normalize_arabic(text):
    """
    تطبيع نص عربي: إزالة bidi/invisible chars والتطويل والتشكيل،
    توحيد الألف/الياء/التاء المربوطة، وتنضيف المسافات والأسطر الزيادة.
    """
    if not text:
        return ""
...
def clean_text(text: str) -> str:
    """تنظيف نص segment واحد: تطبيع عربي (normalize_arabic) ثم الحفاظ على
    علامات الترقيم مع تنظيف المسافات."""
    if not text:
        return ""
